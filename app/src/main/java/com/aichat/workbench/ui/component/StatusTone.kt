...
internal fun statusColors(tone: StatusTone): StatusColors {
    ...
    return when (tone) {
        StatusTone.Neutral -> StatusColors(
            container = scheme.surfaceVariant.copy(alpha = 0.2f),
            content = scheme.onSurfaceVariant,
            border = scheme.outlineVariant.copy(alpha = 0.3f),
        )
        StatusTone.Accent -> StatusColors(
            container = scheme.primary.copy(alpha = 0.06f),
            content = scheme.primary,
            border = scheme.primary.copy(alpha = 0.10f),
        )
        StatusTone.Success -> StatusColors(
            container = scheme.secondary.copy(alpha = 0.06f),
            content = scheme.secondary,
            border = scheme.secondary.copy(alpha = 0.10f),
        )
        StatusTone.Warning -> StatusColors(
            container = scheme.tertiary.copy(alpha = 0.08f),
            content = scheme.tertiary,
            border = scheme.tertiary.copy(alpha = 0.12f),
        )
        StatusTone.Critical -> StatusColors(
            container = scheme.error.copy(alpha = 0.08f),
            content = scheme.error,
            border = scheme.error.copy(alpha = 0.14f),
        )
    }
}
